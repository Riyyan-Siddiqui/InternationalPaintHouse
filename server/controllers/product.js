...
export const createProduct = (req, res) => {
  ...
  if (!product_name || price === undefined || stock_quantity === undefined) {
    return res
      ...
      .json({
        message:
          "Missing required fields (product_name, price, stock_quantity)",
      });
  }
  ...
  if (stock_quantity < 0) {
    return res
      .status(400)
      .json({ message: "Stock quantity must be a positive number" });
  }
...
export const updateProductById = (req, res) => {
  ...
  const {
    ...
    image_url,
    image,
  } = req.body; // Product details from request body
